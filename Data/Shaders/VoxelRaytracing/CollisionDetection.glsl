 ...
 */
bool rayBoxIntersection(vec3 rayOrigin, vec3 rayDirection, vec3 lower, vec3 upper,
        out vec3 entrancePoint, out vec3 exitPoint)
{
 ...
 * For more details see: https://www.siggraph.org//education/materials/HyperGraph/raytrace/rtinter3.htm
 */
bool rayBoxIntersectionRayCoords(vec3 rayOrigin, vec3 rayDirection, vec3 lower, vec3 upper,
        out float tNearOut, out float tFarOut)
{
    float tNear = -1e7, tFar = 1e7; // Infinite values
    for (int i = 0; i < 3; i++) {
        if (!rayBoxPlaneIntersection(rayOrigin[i], rayDirection[i], lower[i], upper[i], tNear, tFar)) {
