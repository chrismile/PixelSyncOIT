...
double ssim(const sgl::BitmapPtr &expected, const sgl::BitmapPtr &observed)
{
    ...
    double rho_x = mean([X,mu_x](int i) -> double { return sqr(X[i] - mu_x); }, N);
    double rho_y = mean([Y,mu_y](int i) -> double { return sqr(Y[i] - mu_y); }, N);
    // Compute the covariance
    double rho_xy = mean([X,mu_x,Y,mu_y](int i) -> double { return (X[i] - mu_x)*(Y[i] - mu_y); }, N);

    return ((2.0 * mu_x * mu_y + c1) * (2 * rho_xy + c2))
           / ((mu_x*mu_x + mu_y*mu_y + c1) * (rho_x*rho_x + rho_y*rho_y + c2));
}
